...
int nhaptien ()         // ham de nguoi dung nap tien vao may ,kiem tra so tien vua nhap, xuat so du
{
    ...
    else
    {
    	system("cls"); 
		 FILE *f;
   					 f = fopen("admin\\lichsunaptien.txt","a");
   					 if (f == NULL)
    				{
   					 printf("loi mo file lichsunaptien.txt");
   					 exit(1);
    				}
					time_t now;// ham thoi gian trong may tinh
    				time(&now);// """"
    				fprintf(f,"\n ban da nap %d vao` luc: %s",n,ctime(&now));             
...
//-----------------------------------------------------------------------------
void lich_su_naptien()  // ham dung de in gia tri lich su ban hang trong file admin\\lichsubansp.txt len man hinh
{
    FILE *f;
    f= fopen("admin\\lichsunaptien.txt", "r");
    if(f==NULL)
    {
    	printf("loi mo file admin\\lichsunaptien.txt ");
    	exit(1);  		// thoat ct vi loi
    }
    char c;
    while ((c = fgetc(f)) != EOF)  // fgetc(f) doc 1 gia tri tu file . fgetc(f) = EOF khi ket thuc file
		...
		case 2:
			{
			   ...
			   if(code == admin())
			   {
			   	...
			   	printf("\n\n\t___________________________________________________________________________________________________________________________________\n\n\t1 - THONG KE DOANH THU.\t\t|\t2 - XEM LICH SU BAN HANG.\t|\t3 - NAP SO LUONG VAO MAY.\t|\t4 - Xem lich su nap tien.");
			   	int choose;
			   	printf("\n\n----> ");
			   	scanf("%d",&choose);
			   	while(choose == 1){
